...
public class Bot extends ListenerAdapter
{
    ...
    public void onMessageReceived(MessageReceivedEvent event)
    {
        ...
        if (msg.equals(/**"!ping"*/ "&Ahri"))
        {
            ...
            channel.sendMessage("I'm Ahri! How can I help you?\nUse &hghelp for info about the hunger games module.\nUse &stream to link streams and see the streamers of the server!\nMore coming soon!").queue();
        }
        else if (msg.equals("&hgjoin")) {
        	if(hg == null) {
        		hg = new HungerManager(channel);
        	}
        	int joined = hg.join(author.getName());
        	if(joined == 1) {
        		channel.sendMessage(author.getName() + " has successfully joined the hunger games!\nOnce all players are joined, use &hgstart to start the game.").queue();
        	} else {
        		channel.sendMessage("You are already in the game, " + author.getName() + "!").queue();
        	}
        	
        	
        }
        else if (msg.equals("&rpgjoin")) {
        	if(rg == null) {
        		rg = new RPGManager(channel);
        	}
        	int joined = rg.joinPlayer(author.getName());
        	if(joined == 1) {
        		channel.sendMessage(author.getName() + " has successfully joined the party!\nOnce all players are joined, use &rpgstart to start the game.").queue();
        	} else {
        		channel.sendMessage("You are already in the party, " + author.getName() + "!").queue();
        	}
        	
        }
        else if (msg.equals("&hgstart")) {
        	if(hg != null) {
        		int result = hg.nextDay();
            	if(result == 0) {
            		hg = null;
            	}
        	}
        ...
        else if (msg.equals("&rpgstart")) {
        	if(rg != null) {
	        		rg.startQuest("TEMPLE");
            	
        	}
        	
        }
        else if (msg.equals("&hgday")) {
        	if(hg != null) {
        		int result = hg.nextDay();
            	if(result == 0) {
            		hg = null;
        ...
        else if (msg.equals("&rpgturn")) {
        	if(rg != null) {
        		int result = rg.doTurn(0);
        		if(result == 0) {
        			rg = null;
        		}
        	}
        ...
        else if (msg.equals("&rpg1")) {
        	if(rg != null && rg.isInGame(author.getName())) {
        		rg.doTurn(1);
        	}
        	
        }
        else if (msg.equals("&rpg2")) {
        	if(rg != null && rg.isInGame(author.getName())) {
        		rg.doTurn(2);
        	}
        	
        }
        else if (msg.equals("&rpg3")) {
        	if(rg != null && rg.isInGame(author.getName())) {
        		rg.doTurn(3);
        	}
        	
        }
        else if (msg.equals("&hgstatus")) {
        	if(hg == null) {
        		channel.sendMessage("No current hunger games running!").queue();
        	} else {
        		channel.sendMessage("There is a hunger games running!").queue();
        	}
        }
        else if (msg.equals("&rpgstatus")) {
        	if(rg == null) {
        		channel.sendMessage("No current RPG running!").queue();
        	} else {
        		channel.sendMessage("There is a RPG running!").queue();
        	}
        }
        else if(msg.equals("&hghelp")) {
        	channel.sendMessage("&hgjoin joins you in the hunger games and starts one if there isn't one.\n&hgstart starts the current games.\n&hgday plays the next day's events.\n&hgstatus checks the current status of the hunger games.\n&hgbots adds bots to the game if you are in need of friends.\n&hgend ends the current hunger games.").queue();
        }
        else if (msg.equals("&hgbots")) {
        	if(hg != null) {
        		int added = hg.addBots();
            	if(added == 1) {
            		channel.sendMessage("Added bots to the game!").queue();
            	} else {
            		channel.sendMessage("Bots already in the game!").queue();
            	}
        	}
        	
        	
        }
        else if (msg.equals("&hgend")) {
        	if(hg != null) {
        		hg = null;
        		channel.sendMessage("Hunger games ended. Join to start a new one!").queue();
        	}
        }
        else if (msg.equals("&rpgend")) {
        	if(rg != null) {
        		rg = null;
        		channel.sendMessage("RPG ended. Join to start a new one!").queue();
        	}
        }
        //stream stuff
        else if (msg.equals("&stream johnny")) {
        	channel.sendMessage("Come watch Johnny's stream at https://www.twitch.tv/riftdog !").queue();
        }
        else if (msg.equals("&stream zack")) {
        	channel.sendMessage("Come watch Zack's stream at https://www.twitch.tv/thesmartwon !").queue();
        }
        else if (msg.equals("&stream henry")) {
        	channel.sendMessage("Come watch Henry's stream at https://www.twitch.tv/allofthelytes !").queue();
        }
        else if (msg.equals("&stream")) {
        	channel.sendMessage("Usage: '&stream <name>'\nCurrent streamers: johnny, zack, henry").queue();
        }
